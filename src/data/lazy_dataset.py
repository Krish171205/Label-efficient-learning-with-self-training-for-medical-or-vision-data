# """
# Lazy-loading ChestMNIST dataset — uses .npy files with true memory mapping.
...
# With this, 4 or 8 workers share the same ~3.67 GB of physical RAM.
...
#                 f"Run 'python convert_to_npy.py' first."
#             )
...
class LazyChestMNIST(Dataset):
    """
    Kaggle-Optimized Dataset: Loads everything into RAM.
    Kaggle has 30GB RAM; your dataset is <4GB. Using disk mmap is 
    slowing you down unnecessarily.
    """
    ...
    def __init__(self, root: str, split: str, transform=None, size: int = 224):
        ...
        img_path = os.path.join(self.npy_dir, f"{split}_images.npy")
        lbl_path = os.path.join(self.npy_dir, f"{split}_labels.npy")
        
        if not os.path.exists(img_path):
            raise FileNotFoundError(f"Run conversion first! Missing: {img_path}")
        
        # KEY CHANGE: Load fully into RAM immediately
        print(f"🚀 Loading {split} images into RAM for maximum speed...")
        self._images = np.load(img_path) 
        self._labels = np.load(lbl_path)
        self._length = self._images.shape[0]
        print(f"✅ {split} set loaded. ({self._length} images)")

    def __len__(self):
        return self._length
    
    def __getitem__(self, idx):
        # Accessing from RAM is now near-instant
